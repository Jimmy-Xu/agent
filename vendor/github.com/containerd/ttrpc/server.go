...
func (s *Server) Serve(ctx context.Context, l net.Listener) error {
	...
	for {
		conn, err := l.Accept()
		logrus.Errorf("Accept connection!")
		if err != nil {
			logrus.WithError(err).Errorf("accept connection failed!")
			select {
			case <-s.done:
...
func (c *serverConn) run(sctx context.Context) {
	...
	go func(recvErr chan error) {
		...
		sendImmediate := func(id uint32, st *status.Status) bool {
			...
			case responses <- response{
				...
				logrus.Errorf("send immediate response")
				return true
			case <-c.shutdown:
				return false
			case <-done:
		...
		for {
			...
			logrus.Errorf("before receive message")

			mh, p, err := ch.recv()
			if err != nil {
				logrus.WithError(err).Errorf("receive failed!")
				...
				if !sendImmediate(mh.StreamID, status) {
					logrus.Errorf("connection closing")
					return
				}
			...
			if mh.Type != messageTypeRequest {
				...
				logrus.Errorf("Wrong message type")
				continue
			}
			...
			logrus.Errorf("request unmarshalled")
	...
	for {
		...
		case request := <-requests:
			active++
			logrus.Errorf("processing request")
			go func(id uint32) {
